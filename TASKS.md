...
62.3.1 [pending] Apply a staged river-gap remediation flow before deeper polygon reconstruction: first add/enable high-detail OSM river rendering as the primary visual fix at high zoom (`z>=12`) for river-gap review hexes, then rerun `reports/river-water-gap-checklist.*` and only perform targeted water-body reconstruction on the remaining unresolved hexes (including disconnected side branches like `HX-W9-N46` toward `HX-W9-N45`) after a review by the user.
