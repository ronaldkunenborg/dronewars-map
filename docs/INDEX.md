# Documentation Index

This project keeps quickstart and high-level context in `README.md`, with detailed topics split into focused documents here.

## Setup

- [Windows OSGeo4W Setup](setup/windows-osgeo4w.md)

## Pipeline

- [Public Layer Builder (`data:layers:public`)](pipeline/public-layer-builder.md)
- [Full Local Layer Builder (`data:layers`)](pipeline/full-local-builder.md)

## Hydrology

- [River Gap Repair Workflow](hydrology/river-gap-repair.md)
- [Coastal Water Correction Workflow](hydrology/coastal-water-correction.md)

## Reports

- [Analytics and Report Commands](reports/analytics-reports.md)

## Data Policy

- [External Sources Policy](data/external-sources.md)
- [Attribution](ATTRIBUTION.md)

## UI

- [Map Layers and Controls](ui/map-layers-and-controls.md)

## Development Process

- [Tasks and Governance](dev/tasks-and-governance.md)

## Decisions (ADRs)

- [ADR-0001: Ukraine Boundary Stack from ADM2 Topology](decisions/ADR-0001-ukraine-boundary-topology.md)
- [ADR-0002: River Gap Repair Uses OSM Lines + Polygon Reconstruction](decisions/ADR-0002-river-gap-repair-approach.md)
